...
import {
  ...
  FABRIC_CORE_DIR, TRANSLATION_AUDIT_DIR, globalPaths,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Auto-run on first activation or version bump (silent, no terminal)
  const lastVersion = context.globalState.get<string>(SETUP_VERSION, '');
  const isFirstRun = lastVersion === '';
  if (lastVersion !== CURRENT_VERSION) {
    runSetupLegacy(context, false).then(() => {
      if (isFirstRun) {
        vscode.commands.executeCommand(
          'workbench.action.openWalkthrough',
          'JasonNicolini.fabric-powerbi-toolkit#fabricPowerbi.getStarted',
          false
        );
      }
    });
...
/**
 * Launch interactive CLI in a VS Code terminal for manual "Full Setup" command.
 */
...
/**
 * Silent setup for auto-run on activation. No terminal, no interactivity.
 * Kept as-is from original for backwards compat.
 */
async function runSetupLegacy(
  ...
  const lastVersion = context.globalState.get<string>(SETUP_VERSION, '');
  const isVersionBump = lastVersion !== '' && lastVersion !== CURRENT_VERSION;
  const forceOverwrite = manual || isVersionBump;
  ...
  await vscode.window.withProgress(
    ...
    async (progress) => {
      if (isVersionBump) {
        outputChannel.appendLine(`Upgrading from v${lastVersion} → v${CURRENT_VERSION}. Bundled files will be refreshed.`);
      }
      ...
      if (!pythonResult.ok) { outputChannel.appendLine(`WARN: ${pythonResult.message}`); }
      if (!uvResult.ok) { outputChannel.appendLine(`WARN: ${uvResult.message}`); }
      ...
      for (const fileName of WORKSPACE_FILES) {
        ...
        if (fs.existsSync(src)) { copyFileIfNewer(src, dest, forceOverwrite); }
      }
      ...
      const daDestDir = path.join(workspaceRoot, '.claude', DEPLOY_AGENTS_SKILL_DIR);
      if (!fs.existsSync(daDestDir)) { fs.mkdirSync(daDestDir, { recursive: true }); }
      for (const fileName of DEPLOY_AGENTS_SKILL_FILES) {
        const src = path.join(daSrcDir, fileName);
        const dest = path.join(daDestDir, fileName);
        if (fs.existsSync(src)) { copyFileIfNewer(src, dest, forceOverwrite); }
      }
      ...
      if (!fs.existsSync(agentsDestDir)) { fs.mkdirSync(agentsDestDir, { recursive: true }); }
      ...
      outputChannel.appendLine(`Setup complete. ${serverCount} MCP server(s) configured.`);
    }
  );
}
