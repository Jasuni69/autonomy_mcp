...
/** Minimal logger interface — vscode.OutputChannel satisfies this naturally */
...
export function checkDotnet(): PrereqResult {
  ...
  if (out) {
    // Look for .NET 9.x+ runtime (9.x or 10.x work for Power BI Modeling MCP)
  ...
  for (const dp of dotnetPaths) {
    if (fs.existsSync(dp)) {
      const fbOut = runCmd(`"${dp}" --list-runtimes`);
...
export async function installFabricCore(
  ...
  try {
    ...
    logger.appendLine(`Copied fabric-core to ${destDir}`);

    // Run uv sync
    logger.appendLine('Running uv sync...');
    ...
    logger.appendLine('uv sync complete');
...
export async function setupAuditVenv(
  ...
  try {
    logger.appendLine(`Creating audit venv with ${systemPython}...`);
    ...
    logger.appendLine('Installing mcp[cli] in audit venv...');
...
function smokeTestProcess(name: string, cmd: string, args: string[]): SmokeTestResult {
  try {
    // Spawn process directly (no shell) — give it 5s, then kill.
    // Just checking it doesn't immediately crash with a bad exit code.
    const proc = cp.spawnSync(cmd, args, {
      timeout: 5000,
      stdio: ['pipe', 'pipe', 'pipe'],
    });

    // status null = still running (killed by timeout) = good, it started.
    // status 0 = exited cleanly. Also good.
    // status non-zero = crashed.
